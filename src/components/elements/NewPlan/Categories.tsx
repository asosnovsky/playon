...
import { List, ListItem, Checkbox, ListItemText, Collapse, ListItemIcon, ListItemSecondaryAction } from '@material-ui/core';
...
class Category extends React.Component<MetaActivityCategory & { 
    ...
    render = () => {
        ...
        return [
            <ListItem button key={this.props.name} onClick={ () => this.setState({ open: !this.state.open }) }
                style={{
...
export default function Categories(props: { 
    ...
    return <List dense component="nav" disablePadding>
        {mockCategories.map( cat => <Category {...cat} selected={props.selected} onSelect={ props.onSelect }/> )}
    </List>
}